...
export async function generateMetadata({ params, searchParams }: PageProps): Promise<Metadata> {
  ...
  if (!board) return {};
  const metadata = buildBoardMetadata(board, 1, locale);
  if (boardId === 'all' || query.trim()) {
    metadata.robots = { index: false, follow: true };
  }
  return metadata;
}

export default async function BoardRoutePage({ params, searchParams }: PageProps) {
  const { boardId } = await params;
  const query = (await searchParams)?.q || '';
  const locale = await getServerLocale();
  ...
  try {
    const threadsPage = await getThreads(boardId, 1, query);
